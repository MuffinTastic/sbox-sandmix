...
namespace NWaves.Utils
{
    ...
    public static class MemoryOperationExtensions
    {
        ...
        {
			var fromSpan = source.AsSpan( sourceOffset, size );
			var toSpan = destination.AsSpan( destinationOffset );
        ...
        public static float[] MergeWithArray(this float[] source, float[] another)
        {
			...
			var toSpan = merged.AsSpan( 0, source.Length );
			var anotherToSpan = merged.AsSpan( source.Length, another.Length );

			source.CopyTo( toSpan );
			another.CopyTo( anotherToSpan );
        ...
        public static float[] RepeatArray(this float[] source, int n)
        {
            ...
            for (var i = 0; i < n; i++)
            {
				var toSpan = repeated.AsSpan( offset );
				source.CopyTo( toSpan );
